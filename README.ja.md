...
**DeepSeek Harness エージェントのスキル(Skills)・MCP サーバー・ツール(Tools)を管理:今まさに到達できるものを可視化し、セッション単位・プリセット単位で切り替える。**

スキル & MCP 管理パネル:すべてのスキル・MCP サーバー・システムツールについて、実際にコンテキスト内にあるかどうかの状態と、次のモデルステップから即座に効くスイッチを備えます。
...
| | |
|---|---|
| 概要 | [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)(`dsh`)の web プラグイン。スキル & MCP 管理パネルとして、現在のセッションのスキル(skills)・MCP サーバー(MCP servers)・システムツール(tools)と、その真のコンテキスト内状態を一覧し、個別に切り替える |
...
| 要件 | dsh web プロファイル、dsh ≥ 0.1.2-alpha.4(それ以前でも動作しますが、ロード状態は degraded 表示。0.1.7 以降ではホスト API の変更に伴いプリセットフォルダを開く機能は利用不可)。`@deepseek-ai/*` の peer はすべてホストが提供 |
| データ | 設定セクション: dsh 0.1.7+ ではアクティブな profile パッチ内の本プラグインエントリ自身の Config(エントリ id でアドレス)、≤ 0.1.6 では `$DSH_HOME/settings.yaml` の `capability-panel` 名前空間。統計は `$DSH_HOME/capability-panel/stats.jsonl`。loopback API `/api/capability-panel` |
...
- **プリセットのデフォルト。** 設定 → 能力パネル で、エージェントプリセットごとのデフォルト能力セットを保存。以後に作成・復元されるセッションがそれを継承します。全部 / スキル / MCP / ツールのカテゴリ切り替え、折りたたみ可能なグループ、セッションパネルと同じフィルタとスイッチを備え、プリセットのデフォルトはあくまで起点で、セッション内で上書きできます。
...
- **充実したテスト**: 580+ のテスト。typecheck・型認識 lint・100% カバレッジゲート(ステートメント/分岐/関数/行)を CI で push と PR のたびに強制。
...
- **ローカルファースト、ネットワーク不要**: データルートは loopback のみを受け付け、プラグインは外部呼び出し・テレメトリ・サードパーティサービスを一切使いません——設定と統計はすべてローカルの `$DSH_HOME` に留まります(「データの保存場所」参照)。
...
- **テーマはタダ**: 色はすべてホストの design token、アイコン・スイッチ・セグメントコントロールはホストのコンポーネント——ライト/ダークや言語切り替えはホストに自動追従し、テーマコードの保守は不要です。
...
DeepSeek Harness の web プロファイル(`dsh web`)、dsh ≥ 0.1.2-alpha.4 が必要です(それ以前のバージョンでもロード状態は degraded 表示で動作します)。1 つのビルドで最新の 0.1.7 まで対応。新ロースターがパスを持たないため「プリセット名からフォルダを開く」機能のみ 0.1.7+ で利用不可で、他の機能は完全に維持されます。`@deepseek-ai/*` の peer はすべてホストが提供するため、他にインストールするものはありません。
...
- 下部の固定フッターバー:左の「全局配置」で設定 → 能力パネル ページへ直行、右の「フィードバック」で issue ページを開きます

`run_code` は予約された Code Mode トランスポートで、レジストリがマスクを禁止しているため、スイッチはオンのまま固定です。

2 つのスコープ、同じスイッチ:**入力欄のパネル**は目の前のセッションに紐づき(再起動後もそのセッションと共に復元)、**設定 → 能力パネル**は以後のすべてのセッションの起点を決めます——フッターの「全局配置」からそのページへ直行できます。
...
- プリセットのデフォルトとセッション単位のスイッチ位置: dsh 0.1.7+ ではアクティブな profile パッチ(`~/.dsh/profiles/<profile>/cordis.patch.yml`、エントリ id でアドレス)内の本プラグインエントリ自身の Config。dsh ≤ 0.1.6 では `$DSH_HOME/settings.yaml` の `capability-panel` 名前空間。セッション単位のスイッチは `sessions.<sessionId>` 配下、最大 200 セッション、古いものから eviction。ハーネスはロードされていないプラグインのセクションを決して削除しないため、アンインストール後も残ります。
