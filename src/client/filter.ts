...
type LabeledSkill = SkillEntry & { readonly stateLabel?: string; readonly sourceLabel?: string };
...
export function filterPayload(
  ...
  sourceLabel: (skill: SkillEntry) => string = (skill) => skill.source,
): FilteredPayload {
  ...
  const skills: LabeledSkill[] = payload.skills.map((skill) => ({ ...skill, stateLabel: stateLabel(skill), sourceLabel: sourceLabel(skill) }));
