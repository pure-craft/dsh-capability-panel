  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === `${ROUTE}/reconnect`) {
        ...
        // Restart the connection and report what it registered. Re-masking the
        // stored defaults is the capability controller's job: it listens to the
        // registry, so the fresh generation is covered the moment it appears.
        const server = record.server;
        const tools = await restartMcpServer(services, server);
        json(res, 200, { ...await presetTools.list(), server, tools });
        return;
      }
