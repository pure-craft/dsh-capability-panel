...
describe('preset tool store', () => {
  it('loads, validates and selects the first preset', async () => {
    ...
    expect(store.getPresetToolsSnapshot().payload?.presets[0]?.tools[1]).toMatchObject({ name: 'run_code', reserved: true });
    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve(response({
      writable: false,
      presets: [{ id: 'plain', name: 'Plain', trust: 'user', broken: 'bad yaml', tools: [] }],
    }))));
    await store.loadPresetTools();
    expect(store.getPresetToolsSnapshot().payload?.presets[0]).toEqual({ id: 'plain', name: 'Plain', trust: 'user', broken: 'bad yaml', tools: [] });
  ...
  it('rejects malformed host payloads rather than showing an empty list', async () => {
    ...
    for (const body of [null, {}, { writable: 'yes', presets: [] }, { writable: true, presets: [null] }, {
      writable: true, presets: [{ id: 1, name: 'x', trust: 'system', tools: [] }],
    }, {
      writable: true, presets: [{ id: 'x', name: 'x', trust: 'other', tools: [] }],
    }, {
      writable: true, presets: [{ id: 'x', name: 'x', trust: 'system', description: 1, tools: [] }],
    }, {
      writable: true, presets: [{ id: 'x', name: 'x', trust: 'system', broken: 1, tools: [] }],
    }, {
      writable: true, presets: [{ id: 'x', name: 'x', trust: 'system', tools: [null] }],
    }, {
      writable: true, presets: [{ id: 'x', name: 'x', trust: 'system', tools: [{ name: 1, enabled: true }] }],
    }, {
      writable: true, presets: [{ id: 'x', name: 'x', trust: 'system', tools: [{ name: 'a', enabled: true, description: 1 }] }],
    }, {
      writable: true, presets: [{ id: 'x', name: 'x', trust: 'system', tools: [{ name: 'a', enabled: true, reserved: 'yes' }] }],
    }]) {
